/**
 * E-S2 Baby Celebration — D-day 카운트다운
 *
 * wedding-minimal과 동일 패턴 (data-bc-* 셀렉터로 격리).
 * data-target ISO 문자열을 읽어 1초마다 days/hours/minutes/seconds 채움.
 */
export const initCountdown = (root: HTMLElement | null): (() => void) => {
  if (!root || typeof window === "undefined") return () => {}

  const container = root.querySelector<HTMLElement>("[data-bc-countdown]")
  if (!container) return () => {}
